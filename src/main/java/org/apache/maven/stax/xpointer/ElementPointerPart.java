...
public class ElementPointerPart implements PointerPart {
     ...
     */
    public ElementPointerPart(String elementID) {
        if (elementID == null) {
     ...
     * @throws IllegalArgumentException if childSequence is empty or contains elements other than Integers.
     */
    public ElementPointerPart(String elementID, List<Integer> childSequence) {
        if (elementID == null) {
            throw new NullPointerException("The elementID argument is null.");
        }
        if (childSequence == null) {
        ...
        if (start < end) { // Make sure sub string is not of zero length.
            try { // Make sure the integer is valid.
                ...
                throw new InvalidXPointerException(
                        "The child sequence part contained a invalid integer.", string, start, end);
            }
        } else {
            throw new InvalidXPointerException(
                    "The child sequence part contained a empty item at " + String.valueOf(start), string, start, end);
        }
    }
}
